...
from lexicon import antwort, no_att_lost, positiv_answer, negative_answer, press_cancel, error
from aiogram.types import Message, ReplyKeyboardRemove
from external_functions import reset, update_table, check_attempts_lost_number, get_secret_number
from aiogram.filters import StateFilter
from aiogram.fsm.context import FSMContext
from bot_states import IN_GAME
...
@game_router.message(DATA_IS_NOT_DIGIT(), F.text.in_(positiv_answer), StateFilter(IN_GAME.after_start))
async def process_positive_answer(message: Message,  state: FSMContext):
    print("positive works")
    user_tg_id = message.from_user.id
    await get_secret_number(user_tg_id)
    await state.set_state(IN_GAME.in_game)
    print("Cтатус установлен")
    await message.answer(text="Я загадал число, начинайте угадывать !",
                         reply_markup=ReplyKeyboardRemove())


# Этот хэндлер будет срабатывать на отказ пользователя сыграть в игру
@game_router.message(F.text.lower().in_(negative_answer), StateFilter(IN_GAME.after_start))
...
@game_router.message(StateFilter(IN_GAME.in_game), DATA_IS_DIGIT())
async def process_numbers_answer(message: Message, state: FSMContext):
    ...
    if await check_attempts_lost_number(user_tg_id):
            ...
            await state.set_state(IN_GAME.after_start)
...
@game_router.message(StateFilter(IN_GAME.after_start))
async def process_error_data(message: Message):
    user_name = message.chat.first_name
